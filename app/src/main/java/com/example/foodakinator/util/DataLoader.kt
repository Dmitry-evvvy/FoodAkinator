...
class DataLoader(private val context: Context) {
    ...
    // Load all data into the database - UPDATED METHOD NAMES
    suspend fun loadDataIntoDatabase(database: FoodAkinatorDatabase) {
        withContext(Dispatchers.IO) {
            try {
                ...
                // Insert data - UPDATED METHOD NAMES
                if (dishes.isNotEmpty()) {
                    database.dishDao().insertAllDishes(dishes)  // CHANGED FROM insertAll
                    Log.d(TAG, "Inserted ${dishes.size} dishes")
                }

                if (questions.isNotEmpty()) {
                    database.questionDao().insertAllQuestions(questions)  // CHANGED FROM insertAll
